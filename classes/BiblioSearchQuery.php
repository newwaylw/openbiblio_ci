...
class BiblioSearchQuery extends Query {
  ...
  /****************************************************************************
   extended function to search 
   Wei Liu

   added @conditions var to restrain search results (such as material type etc.)

   ****************************************************************************/
  function search2($type, $materialCd, $collectionCd, &$words, $page, $sortBy, $opacFlg=true) {
    # reset stats
    $this->_rowNmbr = 0;
    $this->_currentRowNmbr = 0;
    $this->_currentPageNmbr = $page;
    ...
    if ((sizeof($words) == 0) || ($words[0] == "")) {
      ...
      if ($type == OBIB_SEARCH_BARCODE) {
        ...
        $criteria = $this->_getCriteria($type, array("biblio.author","biblio.responsibility_stmt","biblio_field.field_data"),$words);
      } elseif ($type == OBIB_SEARCH_SUBJECT) {
        $criteria = $this->_getCriteria($type, array("biblio.topic1","biblio.topic2","biblio.topic3","biblio.topic4","biblio.topic5"),$words);
      } elseif ($type == OBIB_SEARCH_TITLE) {
        $criteria = $this->_getCriteria($type, array("biblio.title"),$words);
      } else {
          // w.liu extended search...

      }
        #
  
     # echo "<br>criteria = $criteria <br>";
      if ($opacFlg) $criteria = $criteria."and opac_flg = 'Y' ";
    }

      # refined search         
      if("all" != strtolower($materialCd) ){
        $criteria = $criteria."and material_cd = '$materialCd' ";
      }

      if("all" != strtolower($collectionCd) ){
        $criteria = $criteria."and collection_cd = '$collectionCd' ";
      }
 ...
 #  echo "<br>criteria = $criteria <br>";
   ...
   ****************************************************************************
   */
  function _getCriteria($type,$cols,&$words,$bField=false,$drop=0) {
  ...
  function fetchRow() {
    ...
    $bib->setLanguage($array["language"]);
    $bib->setImageLocation($array["image_location"]);
