...
class BiblioQuery extends Query {
  ...
  function doQuery($bibid, $tab="cataloging") {
    ...
    $bib->setOthernumber($array["othernumber"]);
  ...
  function insert($biblio) {
    ...
    $sql = $this->mkSQL("insert into biblio values(null, sysdate(), sysdate(), "
                        ...
                        $biblio->getLanguage(),$biblio->getOthernumber(),
                        $biblio->getImageLocation(),
  ...
  /*********************W.Liu ***********************/
  function updateImageLocation ($biblio) {
    $sql = $this->mkSQL("update biblio set last_change_dt = sysdate(), "
                        ." last_change_userid=%N, image_location=%Q",
                        $biblio->getLastChangeUserid(),
                        $biblio->getImageLocation() );
  ...
  function update($biblio) {
    ...
    $sql = $this->mkSQL("update biblio set last_change_dt = sysdate(), "
                        ...
                        . " collection_cd=%N, "
                        . " call_nmbr1=%Q, call_nmbr2=%Q, call_nmbr3=%Q, language=%Q, othernumber=%Q, ",
                        $biblio->getLastChangeUserid(),
                        $biblio->getMaterialCd(), $biblio->getCollectionCd(),
                        $biblio->getCallNmbr1(), $biblio->getCallNmbr2(),
                        $biblio->getCallNmbr3(), $biblio->getLanguage(), $biblio->getOthernumber());
    
    $image = $biblio->getImageLocation();
    if(!empty($image) ){
        $sql .= $this->mkSQL("image_location=%Q ,",$biblio->getImageLocation());
    }
    foreach ($this->_fieldsInBiblio as $key => $name) {
      if (array_key_exists($key, $biblioFlds) and $biblioFlds[$key]->getFieldid() == '') {
